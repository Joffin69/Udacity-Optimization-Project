...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    //Moved the getElements (pizzaContainer) into a variable outside of the loop so it does bot need to be processed for each element
    // re-size. 
    // the dx and the newwidth is calculated outside the loop as the pizzas are all the same size
...
//Moved the pizzaDiv outside the loop to prevent the DOM query with every iteration.
...
function updatePositions() {
  ...
  //changes the queryselectorall to getelementbyclassid as it takes fewer searches
  //calculation of top variable is taken out of the loop to prevent DOM query on every iteration. Declared phase varibale
  // outside the loop, so it is not created every time the loop is executed.
  var items = document.getElementsByClassName('mover');
  //calculate the items length just once
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  var totalpizzas = (window.screen.height / s) * cols;
